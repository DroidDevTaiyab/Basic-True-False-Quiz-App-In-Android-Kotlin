...
/**
 * Created by Techpass Master on 1-April-21.
...
class PlayActivity : AppCompatActivity() {
    ...
    // create string array for question and answer
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        with(binding) {
            ...
            // show toast if radio button not selected  (if)
            // fun for show next question               (else)
...
//                putExtra("correctAns", correct)
            }
            startActivity(intent)
            Toast.makeText(this, "done", Toast.LENGTH_SHORT).show()
        }

        binding.radioGroup.clearCheck()  // clear radio check
    }
}